...
public class TutorialCARD
{
    ...
    public TutorialCardType m_csCardType;
    ...
    {
        m_cCardGam = cCardGam;
        m_csCardType = m_cCardGam.GetComponent<TutorialCardType>();
...
public class TutorialCardMng : MonoBehaviour {

    private static TutorialCardMng m_Instance = null;
    public static TutorialCardMng I
    {
        get
        {
            if (null == m_Instance)
            {
                m_Instance = FindObjectOfType(typeof(TutorialCardMng)) as TutorialCardMng;
	...
	void Start () {
        ...
        m_cCard = new TutorialCARD[m_nCardNum];

        for (int i = 0; i < m_nCardNum; i++)
        {
            m_cCard[i] = new TutorialCARD();
    ...
    public void Update()
    {
        ...
        if (Tutorial.I.m_nTurn == 7 && Input.GetMouseButtonDown(0))
        {
    ...
    public void CardTwoTurnSuc()
    {
        ...
        if (nCardType1 == nCardType2)
        {
            ...
            if (m_nCardLiveNum == 0)
            {
                ...
                Tutorial.I.m_bClear = true;
                StartCoroutine("RReset");
            }

            if (Random.Range(0, 10) == 7)
            {
                TutorialItemMng.I.CreateItem(m_cCard[nCardIndex1].m_cCardGam.transform.position);
            }
